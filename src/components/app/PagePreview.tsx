...
export function PagePreview({ data }: PagePreviewProps) {
  ...
  return (
    <div
      className="w-full h-full rounded-lg text-white p-8 flex flex-col items-center justify-center text-center relative overflow-hidden"
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-center justify-center gap-4 text-4xl md:text-5xl font-headline font-bold">
          <span>{data.name1 || "Name 1"}</span>
          <Heart className="w-10 h-10 text-rose-300 fill-current" />
          <span>{data.name2 || "Name 2"}</span>
        </div>

        {duration && (
          <div className="mt-4">
            <p className="font-semibold text-lg">Together for</p>
            <p className="text-xl font-bold text-rose-200 tracking-wider">{duration}</p>
          </div>
        )}

        <p className="mt-8 text-lg italic max-w-2xl mx-auto">
          "{data.message || "Your special message goes here."}"
        </p>
